...
@section('titleSuffix')| <a href="{!! route('admin.services.add') !!}" class="text-cyan"><i class="mdi mdi-plus-box"></i> добавить</a>@endsection
@section('content')
    @if(count($items))
        <div class="card">
            <div class="table-responsive">
                <table class="table table-striped m-b-0 columns-middle">
                    ...
                    <tbody class="table-sortable" data-action="{{ route('admin.services.sort') }}">
                    @foreach($items as $item)
                        <tr class="item-row" data-id="{!! $item->id !!}">
                            <td class="item-title bg-success">
                                <a  href="{{ route('admin.services.edit', ['id'=>$item->id]) }}" >
                                    <img src="{{ asset('u/services/'.$item->image) }}" alt="" style="width:40px; height:auto;"></a>
                            </td>
                            <td >
                                <a href="{{ route('admin.services.edit', ['id'=>$item->id]) }}" >
                                {{$item->title}}</a>
                            ...
                            @endif
                            <td>
                                <a href="{{ route('admin.services.edit', ['id'=>$item->id]) }}" {!! tooltip('Редактировать') !!} class="icon-btn edit"></a>
                                <a href="{{ route('admin.sub_service.main', ['parent_id'=>$item->id]) }}" {!! tooltip('sub') !!} class="fa fa-child"></a>
