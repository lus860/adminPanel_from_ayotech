...
@section('titleSuffix')| <a href="{!! route('admin.staff.add') !!}" class="text-cyan"><i class="mdi mdi-plus-box"></i> добавить</a>@endsection
@section('content')
    @if(count($items))
        <div class="card">
            <div class="table-responsive">
                <table class="table table-striped m-b-0 columns-middle">
                    ...
                    <tbody class="table-sortable" data-action="{{ route('admin.staff.sort') }}">
                    @foreach($items as $item)
                         <tr class="item-row" data-id="{!! $item->id !!}">
                            <td class="item-title ">
                                <a  href="{{ route('admin.staff.edit', ['id'=>$item->id]) }}" >
                                    <img src="{{ asset('u/staff/'.$item->image) }}" alt="" style="width:115px; height:auto;"></a>
                            </td>
                            <td >
                                <a href="{{ route('admin.staff.edit', ['id'=>$item->id]) }}" >
                                {{$item->ns}}</a>
                            ...
                            @endif
                            <td>
                                <a href="{{ route('admin.staff.edit', ['id'=>$item->id]) }}" {!! tooltip('Редактировать') !!} class="icon-btn edit"></a>
                                 <a href="{{ route('admin.gallery', ['gallery'=>'staff_item', 'key'=>$item->id]) }}" {!! tooltip('Галерея') !!} class="icon-btn gallery"></a>
